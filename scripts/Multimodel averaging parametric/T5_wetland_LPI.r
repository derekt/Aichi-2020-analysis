...
options(warn = -1)
    ...
    output_name = "T5_wetland_LPI"
  
  # Read in and attach data
  eafs<-read.csv("Wetland_LPI_limited.csv")
  ...
  y_limit = 1.05
  ylabel = "Index value"
  maintitle = "Wetland Extent Index"
  ...
  m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year, FALSE)", 2, "eafs", "list(a =1, b = 1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial")
  ...
  #--------------------------------------------------------------------------------------
  # MODEL 4: HYPERBOLIC
  m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = -87, b = -105)", "list(a = -87, b = -105)", "list(a = -87, b = -105)", TRUE, num_data_points = num_data_points, ylabel, "Hyperbolic")
  ...
  #--------------------------------------------------------------------------------------
  # MODEL 16: ASYMPTOTIC
  m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.55, b = 0.08, d = 1.1)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, ylabel, "Asymptotic", num_data_points = num_data_points, 0.001)
