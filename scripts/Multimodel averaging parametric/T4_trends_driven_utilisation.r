...
options(warn = -1)
  ...
  output_name = "T4_Trends_driven_by_use"

# Read in and attach data
eafs<-read.csv("Trends_driven_by_use_limited.csv")
...
maintitle = "Red List Index showing trends driven by utilisation"
...
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 2, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "2 parameter negative exponential",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 3, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, ylabel, "3 parameter negative exponential", nofit = TRUE, num_data_points = length(eafs[,1]))
...
writepdfsandresults(output_name, 0.83)