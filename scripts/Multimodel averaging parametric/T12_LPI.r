...
eafs<-read.csv("LPI.csv")
eafs = eafs[7:dim(eafs)[1],]
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial")
  ...
  m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 1, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 2000, b = -0.5)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 130, b = -0.1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Monomolecular")

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = -0.1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Ricker")
...
#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 0.2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Weibull")

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -0.8)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Power", 0.1, 0.1, 0.1)
...
writepdfsandresults(output_name, 0.7)