...
setwd("../../data/Target 14")

  output_name = "T14_RLI_pollinators"  

  
  # Read in and attach data
  eafs<-read.csv("RLI_pollinators_limited.csv")
  ...
  y_limit = 0.91
  ylabel = "Index value"
  maintitle = "Red List Index for pollinator birds and mammals"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year, FALSE)", 2, "eafs", "list(a =0.5, b = -0.98)", "coef(m1)", "list(a = 50, b = 5)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial", fitAR2 = FALSE)

 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 0.5, b = 1.1)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "2 parameter negative exponential", fitAR1 = FALSE)
...
writepdfsandresults(output_name, 0.89)