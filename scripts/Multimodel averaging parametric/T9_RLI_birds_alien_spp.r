...
setwd("../../data/Target 9")
  ...
  output_name = "T9_RLI_invasive_birds"

  # Read in and attach data
eafs<-read.csv("RLI_birds_impacted_alien_invasive_spp_limited.csv")
...
# Change predict years now to include all years incuding those with NAs
...
target_value = -99999999999
  y_limit = 0.922
  ylabel = "Red List Index of species survival"
  maintitle = "Red List Index for birds showing trends driven by invasive alien species"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, ylabel, "First order polynomial",fitAR1 = FALSE, num_data_points = length(eafs[,1]))
 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "2 parameter negative exponential",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, ylabel, "3 parameter negative exponential", nofit = TRUE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Hyperbolic",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, ylabel, "Michaelis-Menten",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "list(a = 1.1, b = 0.03)", TRUE, ylabel, "Hollings Type III",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 7: HOLLING TYPE IV
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, c = 1)", TRUE, ylabel, "Hollings Type IV", nofit = TRUE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE, ylabel, "Rational", nofit = TRUE, num_data_points = length(eafs[,1]))

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "Exponential",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 1000, b = 0.1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Monomolecular",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Ricker",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b =0.1,d = 0.1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Gompertz", nofit = TRUE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, ylabel, "Chapman-Richards", nofit = TRUE, num_data_points = length(eafs[,1]))

#---------------------------------------------------------------------------------------
# MODEL 14: 4-PARAMETER GOMPERTZ
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Weibull", nofit = TRUE, num_data_points = length(eafs[,1]))

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", TRUE, ylabel, "Power",fitAR1 = FALSE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Asymptotic", nofit = TRUE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Shepherd", nofit = TRUE, num_data_points = length(eafs[,1]))

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, ylabel, "Hassell", nofit = TRUE, num_data_points = length(eafs[,1]))
...
writepdfsandresults(output_name, 0.919)