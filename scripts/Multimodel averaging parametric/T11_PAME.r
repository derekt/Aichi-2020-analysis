...
options(warn = -1)
  ...
  output_name = "T11_PAME_assessments_per_year" 


# Read in and attach data
eafs<-read.csv("pame_assessments_limited.csv")
...
ylabel = "Cumulative number of sites assessed"
maintitle = "Protected area management effectiveness assessments"
...
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 20000, b = -20, c = 1)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "3 parameter negative exponential", num_data_points = num_data_points)
...
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 40000, b = 50)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Michaelis-Menten", num_data_points = num_data_points)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 100000, b = 0.01, c = 1000)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Hollings Type IV", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.1, b = 50, d = 0.1)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Rational", num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 1, b = 0.21)", "coef(m1)", "coef(m1)", TRUE,  ylabel, "Exponential", num_data_points = num_data_points)
...
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 1, b = 10,d = 5, e= 1)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Weibull", num_data_points = num_data_points)

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Power", num_data_points = num_data_points)
...
#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1800, b = 4, d = 1)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Shepherd", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1800, b = 10, d = 1)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE,  ylabel, "Hassell", num_data_points = num_data_points)
