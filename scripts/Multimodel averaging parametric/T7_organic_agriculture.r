...
setwd("../../data/Target 7")
    ...
    output_name = "T7_Area_organic_agriculture"


# Read in and attach data
eafs<-read.csv("Organic_agriculture.csv")
...
y_limit = max(eafs[,2] * minx) * 1.3
target_value_text = -999999999999
ylabel = "Million hectares"
maintitle = "Area of organic agriculture"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial")
  ...
  m[[3]] <- FitAutocorrelatedModel("rakt4333", "(a,b,c,Year)", 4, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
...
#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b = 2, d = 0.2
)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "Rational")
...
#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 5, b = 5, d = 0.9)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "Asymptotic", 0.1)
