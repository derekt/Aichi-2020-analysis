...
setwd("../../data/Target 20")

  
    output_name = "T20_ODA_biodiversity"


# Read in and attach data
eafs<-read.csv("ODA_biodiv.csv")
...
y_limit = max(eafs[,2]) * minx * 1.5
target_value_text = -999999999999
ylabel = "USD billion, constant 2012 prices"
maintitle = "Bilateral ODA in support of the convention (biodiversity focussed only)"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial", fitAR1 = FALSE)
 
 
  # MODEL 2: 4.2.30 FROM RATKOWSKY
  m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "2 parameter negative exponential", fitAR1 = FALSE)
  
  #-------------------------------------------------------------------------------------
  # MODEL 3: 4.3.33 FROM RATKOWSKY
  m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "3 parameter negative exponential", nofit = TRUE)
  
#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Hyperbolic", fitAR1 = FALSE)

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, num_data_points = num_data_points, ylabel, "Michaelis-Menten", fitAR1 = FALSE)
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = -1)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "Exponential", fitAR1 = FALSE)

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 130, b = -0.1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Monomolecular", fitAR1 = FALSE)

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Ricker", fitAR1 = FALSE)
...
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Chapman-Richards", nofit = TRUE)
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -0.8)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Power", fitAR1 = FALSE)

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Asymptotic", fitAR1 = FALSE)
...
#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Hassell", nofit = TRUE)
...
writepdfsandresults(output_name, 1.0)