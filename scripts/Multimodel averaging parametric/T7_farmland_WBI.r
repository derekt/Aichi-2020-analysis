...
setwd("../../data/Target 7")
    ...
    output_name = "T7_Farmland_WBI"

#--------------------------------------------------------------------------------------
# Read in and attach data
eafs<-read.csv("Farmland_WBI.csv")
...
y_limit = max(eafs[,2]) * 1.2
target_value_text = -999999999999
ylabel = "Index value"
maintitle = "Wild Bird Index for farmland birds"
...
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Chapman-Richards")
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -0.8)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Power", 0.01, 0.1, 0.1)

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Asymptotic", 0.1)
...
#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Hassell")
...
# Now calculate the unconditional variance for the fitted values                                           
...
writepdfsandresults(output_name, 40)