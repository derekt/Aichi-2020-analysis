...
options(warn = -1)
  ...
  output_name = "T2_Research_studies_involving_economic_valuation"
  #--------------------------------------------------------------------------------------

# Read in and attach data
eafs<-read.csv("Number_of_research_studies_involving_economic_valuation.csv")
...
maintitle = "Research studies involving economic valuation"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =1, b = 1.5)", "list(a =1, b = 1.5)", "coef(m1)", TRUE, fitAR2 = FALSE, ylabel, "First order polynomial", num_data_points = num_data_points)
 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 2, "eafs", "list(a = 1, b = 1.5)", "coef(m1)", "coef(m1)", TRUE, fitAR2 = FALSE, ylabel, "2 parameter negative exponential", num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 3, "eafs", "list(a = 1500, b = 10, c = 100)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, ylabel, "3 parameter negative exponential", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Hyperbolic", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, fitAR2 = FALSE, ylabel, "Michaelis-Menten", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "list(a = 1.1, b = 0.03)", TRUE, fitAR2 = FALSE, ylabel, "Hollings Type III", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 7: HOLLING TYPE IV
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, c = 1)", TRUE, fitAR2 = FALSE, ylabel, "Hollings Type IV", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE, fitAR1 = FALSE, ylabel, "Rational", num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = FALSE, ylabel, "Exponential", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 1000, b = 0.1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Monomolecular", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Ricker", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b =0.1,d = 0.1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Gompertz", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Chapman-Richards", num_data_points = num_data_points)

#---------------------------------------------------------------------------------------
# MODEL 14: 4-PARAMETER GOMPERTZ
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 0, b = 1800,d = 4, e= 0.91)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Weibull", num_data_points = num_data_points)

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", TRUE, fitAR2 = FALSE, ylabel, "Power", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1800, b = -1, d = 1.2)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Asymptotic", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1800, b = 4, d = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Shepherd", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1800, b = 10, d = 1)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Hassell", num_data_points = num_data_points)
