...
options(warn = -1)
  ...
  output_name = "T4_water_footprint"


# Read in and attach data
eafs<-read.csv("Water footprint.csv")
...
ylabel = "Thousand km^3"
maintitle = "Water footprint"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, ylabel, "First order polynomial", num_data_points = num_data_points)
...
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 3, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "3 parameter negative exponential", num_data_points = num_data_points)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, c = 1)", TRUE,  ylabel, "Hollings Type IV", num_data_points = num_data_points)
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "Exponential", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 1000, b = 0.1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Monomolecular", num_data_points = num_data_points)
...
#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Weibull", num_data_points = num_data_points)
...
#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Asymptotic", 0.1, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Shepherd", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, fitAR1 = FALSE, ylabel, "Hassell", num_data_points = num_data_points)
