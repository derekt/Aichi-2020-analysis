...
#eafs<-read.csv("PA_coverage_marine_ecoregions.csv")
eafs<-read.csv("PA_coverage_marine_ecoregions_updated.csv")

colnames(eafs) = c("Year", "data")
eafs[,2] = eafs[,2]

  output_name = "T11_marine_ecoregion_coverage" 
...
target_value = -99999999999999999
y_limit = 100
target_value_text = -9999999999999999
ylabel = "Percentage meeting target"
maintitle = "Protected area coverage of marine ecoregions"
...
summary(lm(data ~ Year))
# LM has no significant slope. Just plot LMs 


#-------------------------------------------------------------------------------------
# MODEL 1: FIRST ORDER POLYNOMIAL
# No autocorrelation function
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =0, b = 20)", "coef(m1)", "coef(m1)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "First order polynomial")
 
  # MODEL 2: 4.2.30 FROM RATKOWSKY
  m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "2 parameter negative exponential")
  
  #-------------------------------------------------------------------------------------
  # MODEL 3: 4.3.33 FROM RATKOWSKY
  m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
  

#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 87, b = 0.5)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Hyperbolic")

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Michaelis-Menten")

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 0.8, b = 20)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Hollings Type III")

#--------------------------------------------------------------------------------------
# MODEL 7: HOLLING TYPE IV
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Hollings Type IV")

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.001 , b = 1, d = 1.5)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Rational")

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1.2)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = -0.01)", "coef(m1)", "list(a = 10, b = -0.01)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Monomolecular")

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Ricker")

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b =-0.1,d = 0.1)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Gompertz")

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 40, b = 0.2,d = 0.2)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Chapman-Richards")

#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Weibull")

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 2, b = 2)", "coef(m1)", "list(a = 0.6, b = 0.02)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Power")

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.4, b = -0.1, d = 1.01)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Asymptotic", 0.01)

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Shepherd")

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Hassell")
