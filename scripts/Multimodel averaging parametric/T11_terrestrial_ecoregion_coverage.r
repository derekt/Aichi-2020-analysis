...
#eafs<-read.csv("PA_coverage_terrestrial_ecoregions.csv")
eafs<-read.csv("PA_coverage_terrestrial_ecoregions_updated.csv")
colnames(eafs) = c("Year", "data")
eafs[,2] = eafs[,2]

  output_name = "T11_terrestrial_ecoregion_coverage" 
         minx = min(eafs[,2])                    
# Transform data
...
maintitle = "Protected area coverage of terrestrial ecoregions"
...
#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.001 , b = 1, d = 1.5)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE, num_data_points = num_data_points, ylabel, "Rational")
...
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = -0.01)", "coef(m1)", "coef(m2)",  TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Monomolecular")
