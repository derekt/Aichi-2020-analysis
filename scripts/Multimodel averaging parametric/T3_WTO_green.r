...
options(warn = -1)
    ...
    output_name = "T3_WTO_GreenBox"

  # Read in and attach data
  eafs<-read.csv("WTO_greenboxspending_limited_countries.csv")
  ...
  #y_limit = max(eafs[,2] * minval) * 2.5
  y_limit = max(exp(eafs[,2])) * 2.5
  target_value_text = -999999999999
  ylabel = "USD billion, constant 2010 prices"
  maintitle = "WTO Green Box spending"
  ...
  m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =1, b = 1)", "list(a = 1, b = 1)", "coef(m2)", TRUE, fitAR2 = FALSE, ylabel, "First order polynomial", num_data_points = num_data_points)
   
   
  # MODEL 2: 4.2.30 FROM RATKOWSKY
  m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 2, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, fitAR2 = FALSE, ylabel, "2 parameter negative exponential", 0.05, 0.2, 0.2, num_data_points = num_data_points)
  
  #-------------------------------------------------------------------------------------
  # MODEL 3: 4.3.33 FROM RATKOWSKY
  m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 3, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "3 parameter negative exponential", 0.1, 0.2, 0.4, num_data_points = num_data_points)
  
  #--------------------------------------------------------------------------------------
  # MODEL 4: HYPERBOLIC
  m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 87, b = 0.5)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Hyperbolic", num_data_points = num_data_points)
  ...
  m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Hollings Type III", num_data_points = num_data_points)
  ...
  #-------------------------------------------------------------------------------------
  ## MODEL 9: EXPONENTIAL  
  m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = -1)", "list(a = 1.1, b = -1)", "coef(m2)", TRUE,  ylabel, "Exponential", num_data_points = num_data_points)
  
  #--------------------------------------------------------------------------------------
  # MODEL 10: MONOMOLECULAR
  m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 0.01)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Monomolecular", num_data_points = num_data_points)
  ...
  m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 41, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE,  ylabel, "Chapman-Richards", num_data_points = num_data_points)
  ...
  #---------------------------------------------------------------------------------------
  # MODEL 15: POWER-LAW
  m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 2, b = 2)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Power", num_data_points = num_data_points)
  
  #--------------------------------------------------------------------------------------
  # MODEL 16: ASYMPTOTIC
  m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Asymptotic", num_data_points = num_data_points)
  ...
  #--------------------------------------------------------------------------------------
  # MODEL 18: HASSELL
  m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m1)", "coef(m2)", TRUE,  ylabel, "Hassell", num_data_points = num_data_points)
