...
options(warn = -1)
    ...
    output_name = "T6_Trawl_Effort"

# Read in and attach data
eafs<-read.csv("Trawl_effective_effort.csv")
...
ylabel = "kW sea days"
maintitle = "Trawl effective effort"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "First order polynomial")
  ...
  m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 20, b = 0.1)", "coef(m1)", "coef(m1)", TRUE,  num_data_points = num_data_points, ylabel, "2 parameter negative exponential")
  
  #-------------------------------------------------------------------------------------
  # MODEL 3: 4.3.33 FROM RATKOWSKY
  m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 30, b = -1, c = 2)", "list(a = 30, b = -1, c = 2)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 0.004)", "coef(m1)", "coef(m1)", TRUE, fitAR2 = FALSE,  num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 5000, b = -0.0001)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "Monomolecular")
...
#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 0.01, b = 0.2 ,d = 1)", "coef(m1)", "coef(m2)", TRUE,  num_data_points = num_data_points, ylabel, "Gompertz")
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW 
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m2)", fitAR2 = FALSE, TRUE,  num_data_points = num_data_points, ylabel, "Power")
