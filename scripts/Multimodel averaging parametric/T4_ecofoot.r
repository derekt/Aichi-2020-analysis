...
options(warn = -1)
  ...
  output_name = "T4_ecofoot"

#--------------------------------------------------------------------------------------


# Read in and attach data
eafs<-read.csv("Ecological.footprint.number.earths.demanded_incl.carbon.built.csv")
eafs <-eafs[1:(dim(eafs)[1]),]
colnames(eafs) = c("Year", "data")
minx = min(eafs[,2], na.rm=T)
...
ylabel = "Number of Earths needed"
maintitle = "Ecological footprint"
...
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 2, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "2 parameter negative exponential",  num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 3, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, ylabel, "3 parameter negative exponential",  num_data_points = num_data_points)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Hollings Type IV",  num_data_points = num_data_points)
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Exponential",  num_data_points = num_data_points)
...
#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Asymptotic",  num_data_points = num_data_points)
