...
setwd("../../data/Target 8")
    ...
    output_name = "T8_insecticide_use"


# Read in and attach data
eafs<-read.csv("insecticide_use_cutdown.csv")
...
y_limit = max(back_transform(eafs[,2])) * 1.2
target_value_text = -999999999999
ylabel = "Amount (tonnes)"
maintitle = "Insecticide use"
...
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, num_data_points = num_data_points, ylabel, "Michaelis-Menten",0.1)
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 2, b = -0.01)", "coef(m1)", "coef(m1)", TRUE, num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 2, b = -0.01)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Monomolecular")
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -0.8)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Power")
