...
options(warn = -1)
  ...
  output_name = "T4_cat_1_cites_countries"


# Read in and attach data
eafs<-read.csv("Percent_cat1_nations_CITES.csv")
...
ylabel = "Percent"
maintitle = "Percentage of Category 1 Parties to CITES"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, ylabel, "First order polynomial",fitAR1 = FALSE, num_data_points = num_data_points)
 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("poly2fun", "(a,b,Year)", 2, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "2 parameter negative exponential", fitAR1 = FALSE, num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("poly3fun", "(a,b,c,Year)", 3, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, ylabel, "3 parameter negative exponential", nofit = TRUE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 0.9, b = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Hyperbolic", fitAR1 = FALSE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, ylabel, "Michaelis-Menten",fitAR1 = FALSE, num_data_points = num_data_points)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, c = 1)", TRUE, ylabel, "Hollings Type IV", nofit = TRUE,num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE, ylabel, "Rational", nofit = TRUE,num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m1)", TRUE, ylabel, "Exponential", fitAR1 = FALSE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 0.8, b = -0.2)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Monomolecular", fitAR1 = FALSE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Ricker", fitAR1 = FALSE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b =0.1,d = 0.1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Gompertz", nofit = TRUE,num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, ylabel, "Chapman-Richards", nofit = TRUE,num_data_points = num_data_points)

#---------------------------------------------------------------------------------------
# MODEL 14: 4-PARAMETER GOMPERTZ
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Weibull", nofit = TRUE,num_data_points = num_data_points)

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", TRUE, ylabel, "Power", fitAR1 = FALSE, num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Asymptotic", nofit = TRUE,num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Shepherd", nofit = TRUE,num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, ylabel, "Hassell", nofit = TRUE,num_data_points = num_data_points)
