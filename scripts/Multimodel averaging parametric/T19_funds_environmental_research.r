...
setwd("../../data/Target 19")

output_name = "T19_funds_for_environmental_research"

# Read in and attach data
eafs<-read.csv("funds_for_environmental_research.csv")
...
ylabel = "Constant million USD"
maintitle = "Funds for environmental research"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =1, b = 1.5)", "list(a =1, b = 1.5)", "coef(m1)", TRUE, ylabel, "First order polynomial", num_data_points = num_data_points)
 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 1, b = 1.5)", "coef(m1)", "coef(m1)", TRUE, ylabel, "2 parameter negative exponential", num_data_points = num_data_points)

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 2000, b = 3, c = 0.05)", "coef(m1)", "coef(m2)", TRUE, ylabel, "3 parameter negative exponential", num_data_points = num_data_points)
...
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, ylabel, "Michaelis-Menten", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "list(a = 1.1, b = 0.03)", TRUE, ylabel, "Hollings Type III", num_data_points = num_data_points)

#--------------------------------------------------------------------------------------
# MODEL 7: HOLLING TYPE IV
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 4, b = -2, c = 1)", "coef(m1)", "coef(m2)", TRUE, ylabel, "Hollings Type IV", num_data_points = num_data_points)
...
#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", TRUE, ylabel, "Power", num_data_points = num_data_points)
...
writepdfsandresults(output_name, 1.0, TRUE)