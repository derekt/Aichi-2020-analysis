...
options(warn = -1)
    ...
    output_name = "T11_Terrestrial_PA_coverage"



# Read in and attach data
#eafs<-read.csv("WDPA_limited.csv")
#eafs <-eafs[1:(dim(eafs)[1]),c(1,2)]
eafs<-read.csv("terrestrial_PAs.csv")
colnames(eafs) = c("Year", "data")

# Convert from area to percent
#eafs[,2] = eafs[,2] / (148596902) * 100
minx = min(eafs[,2])
...
target_value = 17
y_limit =      20
target_value_text = 17.5
ylabel = "Percentage land area protected"
maintitle = "Terrestrial protected area"
...
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 1, b = 1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Michaelis-Menten")

#--------------------------------------------------------------------------------------
# MODEL 6: HOLLING TYPE III
m[[6]] <- FitAutocorrelatedModel("hiiifun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Hollings Type III")
...
#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 0.2, b = 0.03)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 1000, b = 0.1)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Monomolecular")
...
#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 0.01, b = 0.33, d = 0.0001, e = 0.01)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Weibull")

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "coef(m1)", "coef(m2)", TRUE, fitAR2 = FALSE, num_data_points = num_data_points, ylabel, "Power")
