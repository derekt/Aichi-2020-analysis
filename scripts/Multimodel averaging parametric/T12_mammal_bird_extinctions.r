...
setwd("../../data/Target 12")

# Turn warnings off for attempting model fits
options(warn = -1)

output_name = "T12_bird_mammal_extinctions"

# Read in and attach data
eafs<-read.csv("mammal_bird_extinctions_update.csv")
...
ylabel = "Number of extinctions during previous 25 years"
maintitle = "Mammal and bird extinctions"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =-22, b = 50)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "First order polynomial")
 
 
# MODEL 2: 4.2.30 FROM RATKOWSKY
m[[2]] <- FitAutocorrelatedModel("ratk423", "(a,b,Year)", 3, "eafs", "list(a = 100, b = 1)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "2 parameter negative exponential")

#-------------------------------------------------------------------------------------
# MODEL 3: 4.3.33 FROM RATKOWSKY
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 100, b = -1, c = 0.5)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "3 parameter negative exponential")



#--------------------------------------------------------------------------------------
# MODEL 4: HYPERBOLIC
m[[4]] <- FitAutocorrelatedModel("hyperfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = -1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Hyperbolic")

#--------------------------------------------------------------------------------------
# MODEL 5: MICHAELIS-MENTEN
m[[5]] <- FitAutocorrelatedModel("mmfun", "(a,b,Year)", 2, "eafs", "list(a = 10, b = 1)", "coef(m1)", "list(a = 1.3, b = 1.3)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Michaelis-Menten",0.1)
...
m[[7]] <- FitAutocorrelatedModel("hivfun", "(a,b,c,Year)", 3, "eafs", "list(a = 1.1, b = 1, c = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, c = 1)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Hollings Type IV")

#--------------------------------------------------------------------------------------
# MODEL 8: RATIONAL
m[[8]] <- FitAutocorrelatedModel("rationalfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b = 1, d = 1)", "coef(m1)", "list(a = 1.1, b = 0.03, d = 1)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Rational")

#-------------------------------------------------------------------------------------
## MODEL 9: EXPONENTIAL  
m[[9]] <- FitAutocorrelatedModel("expfun", "(a,b,Year)", 2, "eafs", "list(a = 20, b = 1)", "coef(m1)", "coef(m1)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Exponential")

#--------------------------------------------------------------------------------------
# MODEL 10: MONOMOLECULAR
m[[10]] <- FitAutocorrelatedModel("monofun", "(a,b,Year)", 2, "eafs", "list(a = 1000, b = 0.1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Monomolecular")

#--------------------------------------------------------------------------------------
# MODEL 11: RICKER
m[[11]] <- FitAutocorrelatedModel("rickerfun", "(a,b,Year)", 2, "eafs", "list(a = 1.1, b = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Ricker")

#--------------------------------------------------------------------------------------
# MODEL 12: GOMPERETZ
m[[12]] <- FitAutocorrelatedModel("gompertzfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1.1, b =0.1,d = 0.1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Gompertz")

#--------------------------------------------------------------------------------------
# MODEL 13: CHAPMAN-RICHARDS
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 12, b = 0.2,d = 0.02)", "list(a = 1.1, b = 0.1,d = 5)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Chapman-Richards")

#---------------------------------------------------------------------------------------
# MODEL 14: WEBIFULL
m[[14]] <- FitAutocorrelatedModel("weibullfun", "(a,b,d,e,Year)", 4, "eafs", "list(a = 2, b = 2,d = 2, e= 0.01)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Weibull")

#---------------------------------------------------------------------------------------
# MODEL 15: POWER-LAW
m[[15]] <- FitAutocorrelatedModel("powerfun", "(a,b,Year)", 2, "eafs", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", "list(a = 200, b = 0.8)", TRUE, fitAR1 = FALSE, num_data_points = num_data_points, ylabel, "Power")

#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 1000, b = 2, d = 1)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Asymptotic")

#--------------------------------------------------------------------------------------
# MODEL 17: SHEPHERD
m[[17]] <- FitAutocorrelatedModel("shepherdfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Shepherd")

#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m2)", TRUE, fitAR1 = FALSE, nofit = TRUE, num_data_points = num_data_points, ylabel, "Hassell")
