# DiagnosticPlots.r modified from: http://ucfagls.wordpress.com/2011/06/12/additive-modelling-and-the-hadcrut3v-global-mean-temperature-series/
  ...
  output_name = "T20_global_funds_environmental_policy_and_laws"
...
eafs<-read.csv("Global_funds_environmental_policy_laws_regulations_economic_instruments.csv")
...
y_limit =      max(eafs[,2]) * min_data * 2
target_value_text = -1000000000000
ylabel = "Constant million USD"
maintitle = "Global funds towards environmental policy and laws"
...
m[[1]] <- FitAutocorrelatedModel("poly1fun", "(a,b,Year)", 2, "eafs", "list(a =1, b = 1)", "list(a = 1, b = 1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "First order polynomial")
...
m[[3]] <- FitAutocorrelatedModel("ratk4333", "(a,b,c,Year)", 4, "eafs", "list(a = 7.5, b = -0.5, c = 2)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "3 parameter negative exponential")
...
m[[13]] <- FitAutocorrelatedModel("chaprichfun", "(a,b,d,Year)", 3, "eafs", "list(a = 41, b = 0.2,d = 0.02)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Chapman-Richards")
...
#--------------------------------------------------------------------------------------
# MODEL 16: ASYMPTOTIC
m[[16]] <- FitAutocorrelatedModel("asymptoticfun", "(a,b,d,Year)", 3, "eafs", "list(a = 2, b = 2, d = 0.98)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Asymptotic")
...
#--------------------------------------------------------------------------------------
# MODEL 18: HASSELL
m[[18]] <- FitAutocorrelatedModel("hassellfun", "(a,b,d,Year)", 3, "eafs", "list(a = 28, b = -0.02, d = 0.-2)", "coef(m1)", "coef(m2)", TRUE, num_data_points = num_data_points, ylabel, "Hassell")
